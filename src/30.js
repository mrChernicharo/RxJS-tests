...
merge(a, b, c, d, e, f, g, h, i, j, k)
	...
	// .pipe(map((val) => val))  // apenas retorna todos os observables ao mesmo tempo sem executá-los. retorna Ajax Observable
	.pipe(
		...
		// exhaustMap((val) => val),
		// mergeMap((val) => val, 2),
		pluck('response', 'data'), // acessa o response.data do objeto AjaxResponse. // printando 'A', 'B', 'C' ...
		combineAll()
	)
